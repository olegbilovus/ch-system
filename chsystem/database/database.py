...
class Database:
    ...
    def execute(self, cur, sql, args=None, repeat=True):
        try:
            cur.execute(sql, args)
            return cur
        except Exception as e:
            self.logger.exception(e)
            if 'server terminated abnormally' in str(e):
                self.logger.info('Getting a new connection')
                self.update_url(force=True)
                if repeat:
                    self.logger.info('Repeating the execution with new connection')
                    cur.close()
                    cur = self.conn.cursor()
                    return self.execute(cur, sql, args, repeat=False)
            else:
                raise e
    ...
    def update_url(self, uri=None, url=None, force=False):
        ...
        if self.conn is None or force:
            ...
            with self.conn.cursor() as cur:
                cur = self.execute(cur, 'SELECT version()')
...
class Server(Database):
    ...
    def get_all(self):
        with self.conn.cursor() as cur:
            cur = self.execute(cur, 'SELECT * FROM server')
            return cur.fetchall()
...
class Clan(Database):
    ...
    def get_by_clan_id(self, clan_id):
        with self.conn.cursor() as cur:
            cur = self.execute(cur, 'SELECT * FROM clan WHERE id = %s', (clan_id,))
            return cur.fetchone()

    def get_server_id_by_clan_id(self, clan_id):
        with self.conn.cursor() as cur:
            cur = self.execute(cur, 'SELECT serverid FROM clan WHERE id = %s', (clan_id,))
    ...
    def update(self, clan_id, clan_name, server_id):
        with self.conn.cursor() as cur:
            cur = self.execute(cur,
                               'UPDATE clan SET name = %s, serverid = %s WHERE id = %s',
                               (clan_name, server_id, clan_id))
            self.conn.commit()
...
class UserProfile(Database):
    ...
    def get_by_id(self, user_id):
        with self.conn.cursor() as cur:
            cur = self.execute(cur, 'SELECT * FROM userprofile WHERE id = %s', (user_id,))
            return cur.fetchone()
    ...
    def update_role(self, user_id, role):
        with self.conn.cursor() as cur:
            cur = self.execute(cur, 'UPDATE userprofile SET role = %s WHERE id = %s', (role, user_id))
            self.conn.commit()
...
class ClanDiscord(Database):
    ...
    def get_all_guild_ids(self):
        with self.conn.cursor() as cur:
            cur = self.execute(cur, 'SELECT discordguildid FROM clandiscord')
            return cur.fetchall()
    ...
    def update(self, clan_id, notify_webhook, discord_guild_id):
        with self.conn.cursor() as cur:
            cur = self.execute(cur, 'UPDATE clandiscord SET notifywebhook = %s, discordguildid = %s WHERE clanid = %s',
...
class Timer(Database):
    ...
    def get_by_clan_id_order_by_type(self, clan_id, preferred_types=None):
        with self.conn.cursor() as cur:
            ...
            if preferred_types is None:
                ...
                cur = self.execute(cur, sql_query, (clan_id, timer, *tuple(preferred_types)))
            return cur.fetchall()
    ...
    def get_num_timers_by_clan_id(self, clan_id):
        with self.conn.cursor() as cur:
            cur = self.execute(cur, 'SELECT count(*) as NUM FROM timer where clanid = %s', (clan_id,))
            return cur.fetchone()
    ...
    def init_timers(self, default_timers, clan_id):
        with self.conn.cursor() as cur:
            ...
            cur = self.execute(cur, sql)
            self.conn.commit()

    def delete(self, clan_id, boss_name):
        with self.conn.cursor() as cur:
            cur = self.execute(cur,
                               'DELETE FROM timer WHERE clanid = %s AND bossName = %s RETURNING *',
                               (clan_id, boss_name))
            self.conn.commit()
            return cur.fetchone()
...
class DiscordID(Database):
    ...
    def get_by_discord_id(self, discord_id):
        with self.conn.cursor() as cur:
            self.execute(cur, 'SELECT * FROM discordid WHERE discordid = %s', (discord_id,))
            return cur.fetchone()

    def insert(self, user_id, discord_id, discord_tag):
        with self.conn.cursor() as cur:
            cur = self.execute(cur,
                               'INSERT INTO discordid (userprofileid, discordid, discordtag) VALUES (%s, %s, %s) RETURNING *',
    ...
    def update(self, user_id, discord_id, discord_tag):
        with self.conn.cursor() as cur:
            cur = self.execute(cur, 'UPDATE discordid SET discordid = %s, discordtag = %s WHERE userprofileid = %s',
...
class Subscriber(Database):
    ...
    def insert(self, user_id, timer_id):
        with self.conn.cursor() as cur:
            cur = self.execute(cur, 'INSERT INTO subscriber (userprofileid, timerid) VALUES (%s, %s)',
                               (user_id, timer_id))
            self.conn.commit()

    def delete(self, user_id, timer_id):
        with self.conn.cursor() as cur:
            cur = self.execute(cur, 'DELETE FROM subscriber WHERE userprofileid = %s AND timerid = %s RETURNING *',
                               (user_id, timer_id))
            self.conn.commit()
            return cur.fetchone()
